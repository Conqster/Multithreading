...
int main()
{
	...
	for (int i = 0; i < 100; ++i)
	{
		...
		else
		{
			...
			{
				PROFILE_SCOPE("Comprehending Frame")
				float duration_ms = time_taken.Duration();
				//{
				//	PROFILE_SCOPE("Clear Console")
				//	system("cls");
				//}
				//DrawASCII(partciles);
				//DrawASCII(cell_partciles);
				//LOG_MSG("Excution duration: " << duration_ms << "ms - " << duration_ms * 0.001 << "s\n");
				LOG_MSG("Particles count: " << partciles.size() << "\n");
				for(const auto& p : cell_partciles)
					LOG_MSG("Particles count: " << p.size() << "\n");
				LOG_MSG("=================================================\n");
